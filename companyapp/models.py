...
class Company(models.Model):
    ...
    user = models.OneToOneField(to=MyUser, on_delete=models.PROTECT, related_name='company', verbose_name="Пользователь", null=True)
    ...
    link = models.CharField('Ссылка на сайт / соц.сеть', max_length=255, blank=True, null=True)
    moder_comment = models.TextField('Заключение модератора', blank=True)
...
class Job(models.Model):
    ...
    moder_comment = models.TextField('Заключение модератора', blank=True, null=True)
    created_at = models.DateTimeField('Дата создания', auto_now_add=True)
    updated_at = models.DateTimeField('Дата обновления', auto_now=True)
    views_count = models.PositiveIntegerField('Число просмотров', default=0)
